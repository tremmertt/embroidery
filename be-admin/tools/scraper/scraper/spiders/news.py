...
class NewsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        news_page_link = self._parse_attribute(
            response, parser_url.selector_type, parser_url.selector
        )
        ...
        if news_page_link:
            ...
            for i, page_link in enumerate(news_page_link):
                yield response.follow(
                    page_link, self.parse_item, cb_kwargs={"index": i}
                )
        else:
            self.total_failed += 1
    ...
    def parse_item(self, response, index):
        ...
        if self.news_thumbnail_link is not None and index < len(
            self.news_thumbnail_link
        ):
        ...
        if parser_title:
            item["title"] = "".join(
                self._parse_attribute(
                    response, parser_title.selector_type, parser_title.selector
                ).getall()
            )
        else:
        ...
        if parser_content:
            item["content"] = "".join(
                self._parse_attribute(
                    response, parser_content.selector_type, parser_content.selector
                ).getall()
            )
        else:
        ...
        if parser_author:
            item["author"] = "".join(
                self._parse_attribute(
                    response, parser_author.selector_type, parser_author.selector
                ).getall()
            )
        else:
        ...
        if parser_category:
            item["category"] = "".join(
                self._parse_attribute(
                    response, parser_category.selector_type, parser_category.selector
                ).getall()
            )
        else:
        ...
        if parser_tag:
            item["tag"] = "".join(
                self._parse_attribute(
                    response, parser_tag.selector_type, parser_tag.selector
                ).getall()
            )
        else:
            item["tag"] = parser_tag
        ...
        if parser_summary:
            item["summary"] = "".join(
                self._parse_attribute(
                    response, parser_summary.selector_type, parser_summary.selector
                ).getall()
            )
        else:
        ...
        if parser_image_link:
            item["image_link"] = "".join(
                self._parse_attribute(
                    response,
                    parser_image_link.selector_type,
                    parser_image_link.selector,
                ).getall()
            )
        else:
