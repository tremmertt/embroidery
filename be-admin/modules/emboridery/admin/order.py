...
def export_selected_objects(message_user, request, queryset):
    ...
    url = f'/emboridery/order/pdf/{list_id[0]}' 
...
class OrderAdmin(ImportExportModelAdmin, admin.ModelAdmin):
    ...
    def download_receipt(self, obj): 
        return mark_safe(
            '<a href="/emboridery/order/pdf/{}" target="_blank" style="width:80px; height:80px;margin: auto !important;padding: auto;" >Download</a>'.format(
                obj.id
            )
        ) 
