    ...
    def add_detail_item_order(self, doc, order_obj): 
        ...
        if list_item_of_order and len(list_item_of_order) != 0: 
            ...
            styles = TableStyle([ 
                ...
                ('FONTNAME', (0,max_data_rows+line_addition),  (max_data_columns+1, max_data_columns+line_addition), 'RobotoBold'),
                ('FONTNAME', (max_data_columns,max_data_rows+1),  (max_data_columns+1, max_data_columns+line_addition), 'RobotoBold'),
                
                ('FONTSIZE', (0,max_data_rows+line_addition),  (max_data_columns+1, max_data_columns+line_addition), 14),

                ('ALIGN', (0,0), (-1, -1), "CENTER"),
                ('VALIGN', (0,0), (-1, -1), "MIDDLE"),
            ])
            data_rows = [
                ['Name', 'H·W·L', 'Format', 'Preview','Quantity', 'Unit Price', 'Sub ({})'.format(self.UNIT_PRICE)]
            ]
            total_quantity = 0
            sub_total_price = 0
            total_price = 0
            for item in list_item_of_order:
                ...
                image = Image(item.image, 0.5*inch, 0.5*inch)
                data_rows.append([item.name, "{}·{}·{}".format(item.height, item.width, item.length), item.output_format, image, item.quantity, item.unit_price, sub_total])
